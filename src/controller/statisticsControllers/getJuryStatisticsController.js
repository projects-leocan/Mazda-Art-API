...
const { short_listed, rejected, scholarship_winner, grant_winner, nominated, submitted, in_review } = require("../../constants/grantConstants");
        ...
        pool.query(query, async (err, result) => {
            if (err) {
                ...
                if (lodash.isEmpty(result.rows)) {
                    ...
                    const getJuryStats = await Promise.all(result.rows.map(async (e) => {
