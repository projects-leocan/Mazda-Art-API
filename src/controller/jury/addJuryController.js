            ...
            if (err) {
                ...
                if (!_.isEmpty(links)) {
                    ...
                    pool.query(linkQuery, async (linkError, linkResult) => {
                        ...
                        if (linkError) {
                            await getJuryDetails(linkResult.rows[0].id, "Jury added success, failed to add links for Jury", res);
                        } else {
                            await getJuryDetails(result.rows[0].id, 'Jury Inserted Successfully', res);
                        }
                    })
                } else {
