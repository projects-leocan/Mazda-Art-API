            ...
            if (submitGrantValidationResult.rowCount === 0 || lodash.isEmpty(submitGrantValidationResult.rows)) {
                ...
                if (
                    ...
                    if (grantAlreadySubmittedResult.rowCount > 0) {
                        if (grantAlreadySubmittedResult.rows[0].submission_updated_count < submitGrantValidationResult.rows[0].max_allow_submision) {
                            // transaction details
                            ...
                            if (is_art_file_updated) {
                                fs.promises.unlink(artistGrantSubmissionFilesPath + grantAlreadySubmittedResult.rows[0].art_file);
                            ...
                            if (art_description != undefined && !lodash.isEmpty(art_description)) {
                            ...
                            pool.query(query, async (err, result) => {
                                ...
                                if (err) {
                                    ...
                                    await getGrantSubmittedDetails(grant_submitted_id, "Grant updated Successfully.", res, req);
                                }
                            });
                            ...
                            res.status(500).send({
                                success: false,
                                message: "Grant can not be update, its reached to max limit of update.",
                                statusCode: 500,
                            });
                        }
                    } else {
